"""Pillar 3, closing the loop on the developing question: when deepseek-v4-flash is
given a LARGE budget (32k) but pushed to k where even that budget cannot complete the
2^k marginalization (k=6,7), is it WRONG-but-still-confident (→ confidence is flat-high
regardless of completion, anti-calibration is the same phenomenon) or WRONG-and-less-
confident (→ it knows when it cannot finish)? This directly tests what the all-correct
in-budget run (run_deepseek.py) could not. No conclusion is final; this develops it.

  source ~/.theone_keys.env && .venv/bin/python experiments/metacognition/run_deepseek_highk.py
"""
...
KS = [6, 7]; N = 8; TOL = 0.005; BUDGET = 32768
...
def main():
    jpath = HERE / "rows_deepseek_highk.jsonl"
    done = set()
    if jpath.exists():
    ...
    print(f"\n=== deepseek-v4-flash, BUDGET={BUDGET}, k={KS} (out-of-reach even at 32k) n={len(have)} ===")
    for k in KS:
    ...
    if not np.isnan(mw):
        print("KEY: deepseek's confidence WHEN WRONG at large budget = "
              f"{mw:.1f} — if ~high, confidence is flat-high decoupled from completion "
              "(anti-calibration = same phenomenon); if low, it knows it cannot finish.")
        (HERE / "results_deepseek_highk.json").write_text(json.dumps(
            {"model": "deepseek-v4-flash", "budget": BUDGET, "ks": KS, "n": len(have),
             "conf_correct": None if np.isnan(mc) else round(float(mc), 1),
             "conf_wrong": round(float(mw), 1), "n_wrong": len(wr)}, indent=2))
