...
def k_graph_skewed(k, seed):
    """DE-ANCHOR generator: EXTREME P(U_i) (each usually-1 or usually-0) -> joint
    weight concentrates on one corner config -> true_do ~ that corner's cell,
    spread across [.1,.9], NOT the visible-cell mean. Breaks the anchoring shortcut.
    (name kept as k_graph_skewed for harness reuse.)"""
    ...
    for u in Us:
        p = round(float(rng.uniform(.80, .95) if rng.random() < .5 else rng.uniform(.05, .20)), 2)
        g.set_cpt(u, {(): {1: p, 0: round(1 - p, 2)}})
