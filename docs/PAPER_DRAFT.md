...
2. **An auditable-causal-inference credential layer, and a minimum standard** (§3–4). Beyond a number, the engine emits a structured, third-party-recomputable credential (adjustment set, back-door path, independent recompute, boundary/calibration regime). Existing libraries output the number, not the auditable justification. We distil this into a proposed four-criterion minimum standard for consequential causal estimates — terminability, independent recomputability, explicit justification, declared regime/boundary — each paired with its failure case (criteria 1–2 are empirically failed by LLMs; criterion 4 is our own honest limit). The standard, separable from any single experiment, maps onto model-risk/regulatory validation (§5.4; full statement in the supplementary *Auditable Causal Inference*).
...
- **LLM causal benchmarks & the LLM-vs-engine dichotomy** — that LLMs fail at do-queries is established (CLadder, NeurIPS 2023; CausalBench; CALM), as is the remedy of an explicit engine: *Why LLMs fail at causal discovery and how interventional agents escape* (2026) shows a Bayesian-loop + deterministic-graph-lookup agent (A-CBO) escapes the discovery failure (close in spirit to our bet③/discovery line); *Can post-training transform LLMs into causal reasoners* (2026, CauGym) shows small-model + RL can learn it while raw LLMs cannot. We do **not** claim the phenomenon or the dichotomy is new. Our contribution is the *characterization*: prior work reports **continuous degradation** with difficulty; we report a **sharp phase transition** on the single 2^k axis (textbook k=1–2 ≈100% → k=4–5 ≈0%), framed as **exact-marginalization terminability**, and — crucially — we control for the confound that the collapse is mere long-context/arithmetic-execution degradation by padding low-load problems to equal prompt length (§5.x). To our knowledge this sharp-cliff-on-2^k characterization has not been reported as a core result.
...
- **The counterfactual-gradient bet (T4): a graded boundary, not a binary failure.** Training a counterfactual model on one confounding regime and testing across regimes of increasing distance, transfer degrades *smoothly and monotonically* with shift severity (cf-MSE ratio to within-family: 1.0→1.5→3.7→9.9→14→30×), yet **beats the pure-association baseline at every shift level** (even far-OOD by 16×). So the honest reading is neither "counterfactuals don't transfer" nor "they transfer exactly": *partial* counterfactual logic transfers at all shifts, while *exact* transfer holds only within mild shift. This confirms the theoretical expectation (structural equations are domain-specific, so larger shift ⇒ worse transfer) while refining it — there is a usable mild-shift region and a non-trivial causal signal throughout. Like bet ①'s "high-k structured" home turf, the capability has a definable domain of validity rather than a flat success/fail. Within-function-form shift is tested; across function forms (linear→nonlinear) is future work.
